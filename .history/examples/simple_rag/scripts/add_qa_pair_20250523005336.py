...
def create_qa_collection():
    ...
    fields = [
        FieldSchema(name="pk", dtype=DataType.VARCHAR, max_length=65535, is_primary=True),
    ...
    # Load the collection to ensure it's ready for search
    collection.load()
    logger.info("Collection 'qa_pairs' loaded successfully")
...
def add_qa_pair(question: str, answer: str, collection: Collection):
    ...
    if embedding_array.shape[0] != 1024:
        raise ValueError(f"Expected embedding dimension 1024, got {embedding_array.shape[0]}")
    
    # Insert into Milvus - using the correct format for each field type
    data = [
        {
            "pk": question,  # Use question as the primary key
            "text": answer,  # Store answer in the 'text' field
            "vector": embedding_array.tolist()  # Convert to list for Milvus
        }
    ]
